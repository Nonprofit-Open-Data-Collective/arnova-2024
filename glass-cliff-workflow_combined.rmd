...
There are two main ways to access the pre-built tables of the data. You can download the data locally to your computer from the data catalog and read it into your R environment or you can use pre-built functions to read it directly in R. We present examples of both below. They are commented out and are just for example purposes
\
...
```{r}
write.csv( partvii, "data/PART-VII-SAMPLE-10-PARSED-NAMES-TITLES.CSV", row.names=F )
```
...
ceo_trans_1000_fncl_wT<- ceo_trans_1000_fncl%>%
  group_by(EIN, transition_no)%>%
...
ceo_trans_1000_fncl_wT$trans_type <- as.factor(ceo_trans_1000_fncl_wT$trans_type)

write.csv(ceo_trans_1000_fncl_wT, "toy_ceo_trans_1000EIN_fncl_wT.csv")
keep <-
  c("EIN","TAXYR","period", "CEO1", "CEO2",  "trans_type")

ceo_trans_1000_fncl_wT[ 1:40, keep ] %>% pander::pander()
```
...
ceo_trans_1000_fncl_MM <- ceo_trans_1000EIN_fncl_wT %>% filter(trans_type == "MM" &
                                             period == "T-1")
ceo_trans_1000_fncl_MF <- ceo_trans_1000EIN_fncl_wT %>% filter(trans_type == "MF"&
                                             period == "T-1")
